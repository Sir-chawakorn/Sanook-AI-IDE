...
/** Installation/setup state of the local Hermes agent. */
export type HermesSetupState =
	/** The `hermes` binary was not found. */
	| 'not_installed'
	/** Installed, but not configured (no model/auth) — the user must run `hermes setup`. */
	| 'needs_setup'
	/** Installed and configured — ready to start sessions. */
	| 'ready';

export interface IHermesStatus {
	readonly state: HermesSetupState;
	/** Resolved path the service would spawn. */
	readonly binaryPath: string;
	/** Hermes home directory inspected for configuration (`~/.hermes`). */
	readonly hermesHome: string;
	/** Human-readable detail for diagnostics / UI. */
...
export interface IHermesAcpService {
	...
	/**
	 * Inspect whether Hermes is installed and configured, without starting a
	 * session. Fast (filesystem heuristic): checks for the binary and for
	 * `~/.hermes/config.yaml` + credentials.
	 */
	checkStatus(binaryPath?: string): Promise<IHermesStatus>;
}
