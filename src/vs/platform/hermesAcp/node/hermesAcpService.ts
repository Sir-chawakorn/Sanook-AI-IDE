...
export class HermesAcpService extends Disposable implements IHermesAcpService {
	...
	async checkStatus(binaryPath?: string): Promise<IHermesStatus> {
		const bin = this._resolveBinary(binaryPath);
		const hermesHome = join(homedir(), '.hermes');

		// `bin` is normally an absolute installer path (~/.local/bin/hermes). If a
		// bare name was provided via $HERMES_BIN we can't stat it, so treat an
		// existing config dir as evidence of an install in that case.
		const binaryFound = existsSync(bin) || (!bin.includes('/') && existsSync(hermesHome));
		if (!binaryFound) {
			return { state: 'not_installed', binaryPath: bin, hermesHome, detail: 'The `hermes` binary was not found.' };
		}

		// Configured = a config file plus at least one credential source.
		const hasConfig = existsSync(join(hermesHome, 'config.yaml'));
		const hasCredentials = existsSync(join(hermesHome, 'auth.json')) || existsSync(join(hermesHome, '.env'));
		if (!hasConfig || !hasCredentials) {
			return { state: 'needs_setup', binaryPath: bin, hermesHome, detail: 'Hermes is installed but not configured. Run `hermes setup`.' };
		}

		return { state: 'ready', binaryPath: bin, hermesHome };
	}
