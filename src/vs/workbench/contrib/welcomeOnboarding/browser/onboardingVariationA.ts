...
const onboardingSteps = defaultChat
	? ONBOARDING_STEPS
	: ONBOARDING_STEPS.filter(stepId => stepId !== OnboardingStepId.SignIn);
 ...
 *
 * Steps:
 * 1. Sign In — optional provider sign-in hero when a product default chat agent is configured
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private readonly steps = onboardingSteps;
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to Sanook AI IDE"));
		...
		this.disposables.add(addDisposableListener(this.nextButton, EventType.CLICK, () => {
			...
			} else if (currentStepId === OnboardingStepId.SignIn) {
	...
	private _updateButtonStates(): void {
		if (this.backButton) {
			const currentStepId = this.steps[this.currentStepIndex];
			const showEnterpriseBack = currentStepId === OnboardingStepId.SignIn && this.enterpriseSignInUiState === 'instance';
			this.backButton.style.display = (this.currentStepIndex === 0 && !showEnterpriseBack) ? 'none' : '';
		}
		if (this.nextButton) {
			const currentStepId = this.steps[this.currentStepIndex];
			if (currentStepId === OnboardingStepId.SignIn) {
		...
		if (this.footerLeft) {
			if (defaultChat && this._isLastStep()) {
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to Sanook AI");
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all Sanook AI commands."),
		);
	}
	...
	private async _applyKeymap(keymapId: string): Promise<void> {
		...
		if (!keymap?.extensionId) {
			return; // Default keymap, nothing to install
		}
