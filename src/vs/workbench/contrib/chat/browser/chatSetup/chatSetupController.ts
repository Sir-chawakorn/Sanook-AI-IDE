...
import { ChatSetupAnonymous, ChatSetupStep, ChatSetupResultValue, InstallChatEvent, InstallChatClassification, refreshTokens, maybeEnableAuthExtension, getDefaultChatAgent } from './chatSetup.js';
...
export class ChatSetupController extends Disposable {
	...
	private async doSetup(options: IChatSetupControllerOptions, watch: StopWatch): Promise<ChatSetupResultValue> {
		...
		if (!defaultChat?.chatExtensionId) {
			this.logService.warn('[chat setup] Skipping setup because no default chat agent is configured.');
			return false;
		}
	...
	private async install(entitlement: ChatEntitlement, watch: StopWatch, options: IChatSetupControllerOptions): Promise<ChatSetupResultValue> {
		const defaultChat = getDefaultChatAgent();
		if (!defaultChat?.chatExtensionId) {
			this.logService.warn('[chat setup] Skipping install because no default chat agent is configured.');
			return false;
		}
	...
	async setupWithProvider(options: IChatSetupControllerOptions): Promise<ChatSetupResultValue> {
		const defaultChat = getDefaultChatAgent();
		if (!defaultChat?.completionsAdvancedSetting || !defaultChat.providerUriSetting) {
			this.logService.warn('[chat setup] Skipping provider setup because no default chat agent is configured.');
			return false;
		}
	...
	private async handleEnterpriseInstance(): Promise<ChatSetupResultValue> {
		const defaultChat = getDefaultChatAgent();
		if (!defaultChat?.providerUriSetting) {
			return false;
		}
