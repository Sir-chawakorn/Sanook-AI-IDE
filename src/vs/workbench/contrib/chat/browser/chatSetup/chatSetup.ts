...
export function getDefaultChatAgent(): IDefaultChatAgent | undefined {
	return product.defaultChatAgent as IDefaultChatAgent | undefined;
}
...
export async function maybeEnableAuthExtension(
	...
	const providerExtensionId = getDefaultChatAgent()?.providerExtensionId;
	if (!providerExtensionId) {
		return false;
	}

	const providerExtension = extensionsWorkbenchService.local.find(
		e => ExtensionIdentifier.equals(e.identifier.id, providerExtensionId)
	);
	...
	if (
		...
		logService.info(`[chat setup] auth provider extension '${providerExtensionId}' is disabled, re-enabling it`);
		try {
			...
			logService.error(`[chat setup] failed to re-enable auth provider extension '${providerExtensionId}'`, error);
			return false;
		}
	}
