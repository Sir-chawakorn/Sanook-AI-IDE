...
import { IChatSetupResult, ChatSetupAnonymous, InstallChatEvent, InstallChatClassification, ChatSetupStrategy, ChatSetupResultValue, getDefaultChatAgent } from './chatSetup.js';
...
export class ChatSetup {
	...
	private async doRun(options?: { disableChatViewReveal?: boolean; forceSignInDialog?: boolean; additionalScopes?: readonly string[]; forceAnonymous?: ChatSetupAnonymous; dialogIcon?: ThemeIcon; dialogTitle?: string; setupStrategy?: ChatSetupStrategy; disableCloseButton?: boolean; onSignInStarted?: () => void }): Promise<IChatSetupResult> {
		...
		if (!getDefaultChatAgent()?.chatExtensionId) {
			this.logService.warn('[chat setup] Skipping setup dialog because no default chat agent is configured.');
	...
	private getButtons(options?: { forceSignInDialog?: boolean; forceAnonymous?: ChatSetupAnonymous }): Array<[string, ChatSetupStrategy, { styleButton?: (button: IButton) => void } | undefined]> {
		const defaultChat = getDefaultChatAgent();
		if (!defaultChat) {
			return [];
		}
		...
		if (!options?.forceAnonymous && (this.context.state.entitlement === ChatEntitlement.Unknown || options?.forceSignInDialog)) {
			const defaultProviderButton: ContinueWithButton | undefined = defaultChat.provider?.default?.name ? [localize('continueWith', "Continue with {0}", defaultChat.provider.default.name), ChatSetupStrategy.SetupWithoutEnterpriseProvider, styleButton('continue-button', 'default')] : undefined;
			const defaultProviderLink: ContinueWithButton | undefined = defaultProviderButton ? [defaultProviderButton[0], defaultProviderButton[1], styleButton('link-button')] : undefined;

			const enterpriseProviderButton: ContinueWithButton | undefined = defaultChat.provider?.enterprise?.name ? [localize('continueWith', "Continue with {0}", defaultChat.provider.enterprise.name), ChatSetupStrategy.SetupWithEnterpriseProvider, styleButton('continue-button', 'default')] : undefined;
			const enterpriseProviderLink: ContinueWithButton | undefined = enterpriseProviderButton ? [enterpriseProviderButton[0], enterpriseProviderButton[1], styleButton('link-button')] : undefined;

			const googleProviderButton: ContinueWithButton | undefined = defaultChat.provider?.google?.name ? [localize('continueWith', "Continue with {0}", defaultChat.provider.google.name), ChatSetupStrategy.SetupWithGoogleProvider, styleButton('continue-button', 'google')] : undefined;
			const appleProviderButton: ContinueWithButton | undefined = defaultChat.provider?.apple?.name ? [localize('continueWith', "Continue with {0}", defaultChat.provider.apple.name), ChatSetupStrategy.SetupWithAppleProvider, styleButton('continue-button', 'apple')] : undefined;
	...
	private createDialogFooter(disposables: DisposableStore, options?: { forceAnonymous?: ChatSetupAnonymous }): HTMLElement {
		...
		const defaultChat = getDefaultChatAgent();
		if (!defaultChat?.provider?.default?.name || !defaultChat.termsStatementUrl || !defaultChat.privacyStatementUrl) {
			return element;
		}

		let footer: string;
		if (options?.forceAnonymous || this.telemetryService.telemetryLevel === TelemetryLevel.NONE || !defaultChat.publicCodeMatchesUrl) {
...
export function refreshTokens(commandService: ICommandService): void {
	...
	const command = getDefaultChatAgent()?.chatRefreshTokenCommand;
	if (!command) {
		return;
	}

	commandService.executeCommand(command).catch(() => { /* command may not be registered */ });
}
