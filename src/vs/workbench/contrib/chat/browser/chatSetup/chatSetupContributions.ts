...
import { buildUpgradeUrlWithRedirect, ChatSetupAnonymous, ChatSetupStrategy, refreshTokens, getDefaultChatAgent } from './chatSetup.js';
...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		if (!context || !requests || !getDefaultChatAgent()?.chatExtensionId) {
			return; // disabled
		}
	...
	private async checkExtensionInstallation(context: ChatEntitlementContext): Promise<void> {
		const chatExtensionId = getDefaultChatAgent()?.chatExtensionId;
		if (!chatExtensionId) {
			return;
		}
		...
		if (this.environmentService.isExtensionDevelopment) {
			...
			if (this.extensionService.extensions.find(ext => ExtensionIdentifier.equals(ext.identifier, chatExtensionId))) {
		...
		this._register(Event.runAndSubscribe<IExtension | undefined>(this.extensionsWorkbenchService.onChange, e => {
			if (e && !ExtensionIdentifier.equals(e.identifier.id, chatExtensionId)) {
				return; // unrelated event
			}

			const defaultChatExtension = this.extensionsWorkbenchService.local.find(value => ExtensionIdentifier.equals(value.identifier.id, chatExtensionId));
...
class ChatSetupExtensionUrlHandler implements IExtensionUrlHandlerOverride {
	...
	canHandleURL(url: URI): boolean {
		const chatExtensionId = getDefaultChatAgent()?.chatExtensionId;
		return !!chatExtensionId && url.scheme === this.productService.urlProtocol && equalsIgnoreCase(url.authority, chatExtensionId);
	}
...
export class ChatTeardownContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		const context = chatEntitlementService.context?.value;
		if (!context || !getDefaultChatAgent()?.chatExtensionId) {
			return; // disabled
		}
