...
import { ChatSetupAnonymous, ChatSetupStep, IChatSetupResult, maybeEnableAuthExtension, refreshTokens, getDefaultChatAgent } from './chatSetup.js';
...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatModeKind, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			const name = defaultChat ? `${defaultChat.provider.default.name} Copilot` /* Do NOT change, this hides the username altogether in Chat */ : localize('setupAgentName', "AI Features");
			return SetupAgent.doRegisterAgent(instantiationService, chatAgentService, id, name, true, description, location, mode, context, controller);
		});
	}
	...
	private registerCommands(): void {
		...
		this._register(CommandsRegistry.registerCommand(SetupAgent.CHAT_SHOW_OUTPUT_COMMAND_ID, async (accessor) => {
			...
			const defaultChat = getDefaultChatAgent();
			const outputExtensionStateCommand = defaultChat?.chatExtensionOutputExtensionStateCommand;
			const outputChannelId = defaultChat?.chatExtensionOutputId;

			if (outputExtensionStateCommand) {
				...
				raceTimeout(
					commandService.executeCommand(outputExtensionStateCommand),
					5000,
			...
			if (outputChannelId) {
				await commandService.executeCommand(`workbench.action.output.show.${outputChannelId}`);
			}
		}));
	}
	...
	private async doForwardRequestToChatWhenReady(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		...
		if (whenLanguageModelReady instanceof Promise || whenAgentReady instanceof Promise || whenToolsModelReady instanceof Promise) {
			...
			try {
				...
				if (ready === 'timedout') {
					const defaultChat = getDefaultChatAgent();
					const chatExtensionId = defaultChat?.chatExtensionId;
					let warningMessage: string;
					if (this.chatEntitlementService.anonymous && chatExtensionId) {
						warningMessage = localize('chatTookLongWarningAnonymous', "Chat took too long to get ready. Please ensure that the extension `{0}` is installed and enabled. Click restart to try again if this issue persists.", chatExtensionId);
					} else if (defaultChat && chatExtensionId) {
						warningMessage = localize('chatTookLongWarning', "Chat took too long to get ready. Please ensure you are signed in to {0} and that the extension `{1}` is installed and enabled. Click restart to try again if this issue persists.", defaultChat.provider.default.name, chatExtensionId);
					} else {
						warningMessage = localize('chatTookLongWarningNoDefaultAgent', "Chat took too long to get ready. Click restart to try again if this issue persists.");
					}
					...
					const outputChannelId = defaultChat?.chatExtensionOutputId;
					if (outputChannelId && this.outputService.getChannelDescriptor(outputChannelId)) {
						progress({
							kind: 'command',
							command: {
						...
						this.logService.warn(outputChannelId
							? `[chat setup] No output channel found for id '${outputChannelId}' to show details about chat setup timeout. Please ensure the ${chatExtensionId ?? 'default chat'} extension is activated.`
	...
	private replaceAgentInRequestModel(requestModel: IChatRequestModel, chatAgentService: IChatAgentService): IChatRequestModel {
		...
		const extensionId = getDefaultChatAgent()?.extensionId;
		if (!extensionId) {
			return requestModel;
		}

		const agentId = agentPart.agent.id.replace(/setup\./, `${extensionId}.`.toLowerCase());
