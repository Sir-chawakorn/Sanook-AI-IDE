...
export function registerChatActions() {
	...
	if (defaultChatAgent?.completionsAdvancedSetting && defaultChatAgent.provider.enterprise.id) {
		const nonEnterpriseCopilotUsers = ContextKeyExpr.and(ChatContextKeys.enabled, ContextKeyExpr.notEquals(`config.${defaultChatAgent.completionsAdvancedSetting}.authProvider`, defaultChatAgent.provider.enterprise.id));
	...
	if (defaultChatAgent?.completionsMenuCommand) {
		registerAction2(class ConfigureCopilotCompletions extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				const command = product.defaultChatAgent?.completionsMenuCommand;
				if (!command) {
					return;
				}
