...
class QuickChat extends Disposable {
	...
	private setupDisclaimer(parent: HTMLElement): void {
		...
		this._register(autorun(reader => {
			...
			const showDisclaimer = !!defaultChatAgent && !sentiment.completed && anonymous && !requestInProgress;
			disclaimerElement.classList.toggle('hidden', !showDisclaimer);

			if (showDisclaimer) {
				const renderedMarkdown = disposables.add(this.markdownRendererService.render(new MarkdownString(localize({ key: 'termsDisclaimer', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} Copilot, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChatAgent.provider.default.name, defaultChatAgent.provider.default.name, defaultChatAgent.termsStatementUrl, defaultChatAgent.privacyStatementUrl), { isTrusted: true })));
