...
import { getDefaultChatAgent } from '../chatSetup/chatSetup.js';
...
export class ChatStatusDashboard extends DomWidget {
	...
	private render(): void {
		...
		const hasInlineSuggestionsSettings = this.hasInlineSuggestionsSettings();
		const hasQuickSettingsContent =
			(hasInlineSuggestionsSettings && !this.options?.disableInlineSuggestionsSettings) ||
	...
	private renderInlineSuggestionsSection(hasContentAbove: boolean): void {
		...
		if (statusEl) {
			const completionsEnablementSetting = getDefaultChatAgent()?.completionsEnablementSetting;
			this._store.add(this.configurationService.onDidChangeConfiguration(e => {
				if (completionsEnablementSetting && e.affectsConfiguration(completionsEnablementSetting)) {
	...
	private renderInlineSuggestionsContent(container: HTMLElement): void {
		...
		if (!this.options?.disableInlineSuggestionsSettings && this.hasInlineSuggestionsSettings()) {
	...
	private hasInlineSuggestionsSettings(): boolean {
		const defaultChat = getDefaultChatAgent();
		return !!(defaultChat?.completionsEnablementSetting && defaultChat.nextEditSuggestionsSetting);
	}
	...
	private createSettings(container: HTMLElement): void {
		const defaultChat = getDefaultChatAgent();
		if (!defaultChat?.completionsEnablementSetting || !defaultChat.nextEditSuggestionsSetting) {
			return;
		}
	...
	private createInlineSuggestionsSetting(container: HTMLElement, label: string, modeId: string | undefined): void {
		const settingId = getDefaultChatAgent()?.completionsEnablementSetting;
		if (!settingId) {
			return;
		}

		this.createSetting(container, [settingId], label, this.getCompletionsSettingAccessor(modeId));
	}
	...
	private createNextEditSuggestionsSetting(container: HTMLElement, label: string, completionsSettingAccessor: ISettingsAccessor): void {
		...
		if (!nesSettingId || !completionsSettingId) {
			return;
		}
