...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(Event.runAndSubscribe(
			...
			() => {
				...
				const chatExtension = notification?.extensions.find(ext => ExtensionIdentifier.equals(ext.identifier.id, chatExtensionId));
				if (chatExtension) {
					isInvalid.set(true);
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const commandButton = `[${showExtensionLabel}](${createCommandUri(showExtensionsWithIdsCommandId, [chatExtensionId])})`;
