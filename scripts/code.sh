...
function code-wsl()
{
	...
	# in a wsl shell
	ELECTRON_NAME=$(cd "$ROOT" && node -p "require('./product.json').win32NameVersion || require('./product.json').nameLong")
	ELECTRON="$ROOT/.build/electron/$ELECTRON_NAME.exe"
	if [ -f "$ELECTRON"  ]; then
		local CWD=$(pwd)
		cd $ROOT
		...
		if [ -n "$WSL_EXT_WLOC" ]; then
			...
			echo "Remote WSL not installed, trying to run Sanook AI in WSL."
		fi
	fi
}
