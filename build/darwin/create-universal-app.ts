...
const darwinArchPlatforms = ['darwin-x64', 'darwin-arm64'];
...
function getSingleDarwinArchBuiltInExtensions(product: IProductJson): string[] {
	...
	for (const extension of extensions) {
		...
		const darwinArchPlatformCount = darwinArchPlatforms.filter(platform => platforms.has(platform)).length;
		if (darwinArchPlatformCount > 0 && darwinArchPlatformCount < darwinArchPlatforms.length) {
...
async function main(buildDir?: string) {
	...
	const product = JSON.parse(fs.readFileSync(path.join(root, 'product.json'), 'utf8')) as IProductJson;
	...
	const singleDarwinArchBuiltInExtensions = getSingleDarwinArchBuiltInExtensions(product);
	...
	for (const extensionName of singleDarwinArchBuiltInExtensions) {
		crossCopyPlatformDir(x64AppPath, arm64AppPath, path.join('Contents', 'Resources', 'app', 'extensions', extensionName));
	}
	...
	const x64ArchFiles = [
		...
		'**/node_modules/@github/copilot-darwin-*/copilot',
		...
		'**/node_modules.asar.unpacked/@github/copilot-darwin-*/copilot',
		...
		'**/node_modules/@microsoft/mxc-sdk/bin/**',
		'**/node_modules.asar.unpacked/@microsoft/mxc-sdk/bin/**',
		...singleDarwinArchBuiltInExtensions.map(extensionName => `**/extensions/${extensionName}/**`),
	];
	...
	await makeUniversalApp({
		...
		force: true,
		mergeASARs: true,
		x64ArchFiles: `{${x64ArchFiles.join(',')}}`,
