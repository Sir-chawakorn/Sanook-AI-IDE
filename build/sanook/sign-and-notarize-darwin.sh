...
if [[ -n "${APPLE_ID:-}" && -n "${APPLE_TEAM_ID:-}" && -n "${APPLE_APP_SPECIFIC_PASSWORD:-}" ]]; then
  ...
  if [[ "${SANOOK_ALLOW_SIGN_WITHOUT_NOTARIZATION:-}" = "1" ]]; then
    echo "APPLE_ID / APPLE_TEAM_ID / APPLE_APP_SPECIFIC_PASSWORD not set; signed but skipped notarization because SANOOK_ALLOW_SIGN_WITHOUT_NOTARIZATION=1."
  else
    echo "APPLE_ID / APPLE_TEAM_ID / APPLE_APP_SPECIFIC_PASSWORD not set; refusing to produce an unnotarized release artifact." >&2
    echo "Set SANOOK_ALLOW_SIGN_WITHOUT_NOTARIZATION=1 only for local non-release testing." >&2
    exit 2
  fi
fi
