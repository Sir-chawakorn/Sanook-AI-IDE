...
import { getCopilotExcludeFilter, getCopilotRuntimePrebuildFiles, getCopilotTgrepExcludeFilter, getRipgrepExcludeFilter } from './lib/copilot.ts';
...
const minifyVSCodeTask = task.define('minify-vscode', task.series(
	...
	optimize.minifyTask('out-vscode')
));
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		const platformSpecificBuiltInExtensionsExclusions = product.builtInExtensions.filter(ext => {
			...
			return !set.has(platform) && !set.has(`${platform}-${arch}`);
		...
		const deps = es.merge(cleanedDeps, copilotRuntimePrebuilds)
			...
			.pipe(jsFilter)
			.pipe(stripSourceMapsInPackagingTasks ? util.stripSourceMappingURL() : es.through())
	...
	return async () => {
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await rcedit(fullPath, {
				...
				'version-string': {
					'CompanyName': product.nameShort,
					'FileDescription': product.nameLong,
					'FileVersion': packageJson.version,
					'InternalName': basename,
					'LegalCopyright': `Copyright (C) 2026 ${product.nameShort}. Portions Copyright (C) Microsoft Corporation.`,
