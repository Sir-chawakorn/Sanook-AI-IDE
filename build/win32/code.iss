...
AppPublisher={#NameShort}
AppPublisherURL=https://github.com/Sir-chawakorn/sanook-ai-ide
AppSupportURL=https://github.com/Sir-chawakorn/sanook-ai-ide/issues
AppUpdatesURL=https://github.com/Sir-chawakorn/sanook-ai-ide/releases
DefaultGroupName={#NameLong}
AllowNoIcons=yes
OutputDir={#OutputDir}
OutputBaseFilename=SanookAISetup
Compression=lzma
...
// to shutdown the app. We rely on the fact that the update signals
...
begin
  Result := True;
    ...
    if not WizardSilent() and IsAdmin() then begin
      if MsgBox('This User Installer is not meant to be run as an Administrator. If you would like to install {#NameShort} for all users in this system, use the System Installer instead. Are you sure you want to continue?', mbError, MB_OKCANCEL) = IDCANCEL then begin
        Result := False;
      end;
    end;
  #endif
...
// The app will create a flag file before the update starts (/update=C:\foo\bar)
// - if the file exists at this point, the user quit the app before the update finished, so don't start the app after update
// - otherwise, the user has accepted to apply the update and the app should start
function LockFileExists(): Boolean;
begin
  Result := FileExists(ExpandConstant('{param:update}'))
end;

// Check if the app created a session-end flag file to indicate OS is shutting down
...
// Check if the app created a cancel file to signal that the update should be aborted
...
begin
  ...
  // During background updates, also create a -updating mutex that the app checks
