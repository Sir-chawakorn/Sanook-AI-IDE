...
export function hygiene(some: NodeJS.ReadWriteStream | string[] | undefined, runEslint = true): NodeJS.ReadWriteStream {
	...
	const productJson = es.through(function (file: VinylFile) {
		// The Sanook AI fork intentionally ships an extensionsGallery (open-vsx) in
		// product.json. Upstream Code-OSS forbids this, but the fork relies on it for
		// extension installation, so the check is intentionally omitted here.
		this.emit('data', file);
	});
